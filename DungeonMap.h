...
class DungeonMap
{
	...
	// coordinate related
	irr::core::vector2di startPosition;
	...
	void loadDungeonFile(std::string &mapPath);

public:
	DungeonMap(irr::IrrlichtDevice &device, std::string &mapPath);

	// initialize the scene, add nodes to the scenemanager
	void initializeScene();

	// get the start position of the player
	irr::core::vector2di getStartPosition();
